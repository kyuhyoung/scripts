...
echo "==============================================================================================================================="
echo "USAGE : docker build --force-rm -t ${docker_name} -f Dockerfile_${docker_name}; docker run --rm -it -w \$PWD -v \$PWD:\$PWD \${docker_name} bash docker_file/extract_docker_info.sh"
echo "-------------------------------------------------------------------------------------------------------------------------------"
...
echo "opencv : ${cv}"
tf=$(pip list | grep tensorflow | rev | xargs | cut -d' ' -f1 | rev)
echo "tensorflow : ${tf}"
torch=$(pip list | grep "torch " | rev | xargs | cut -d' ' -f1 | rev)
echo "pytorch : ${torch}"
tv=$(pip list | grep torchvision | rev | xargs | cut -d' ' -f1 | rev)
echo "torchvison : ${tv}"
paddle=$(pip list | grep paddlepaddle | rev | xargs | cut -d' ' -f1 | rev)
echo "paddlepaddle : ${paddle}"
trt=$(dpkg -l | grep TensorRT | cut -d' ' -f3) 
echo "tensorRT : ${trt}"
tb=$(pip list | grep "tensorboard " | rev | xargs | cut -d' ' -f1 | rev)
echo "tensorboard : ${tb}"
bazel=$(which bazel)
echo "bazel : ${bazel}"
echo "==============================================================================================================================="
